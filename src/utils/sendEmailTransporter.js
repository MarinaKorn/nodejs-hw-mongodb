...
const transporter = nodemailer.createTransport({
  host: getEnvVar('SMTP_HOST') || 'smtp.ukr.net',
  port: getEnvVar('SMTP_PORT') || 465,
  secure: true,
  auth: {
    user: getEnvVar('SMTP_USER') || 'marynakorniyenko@ukr.net',
    pass: getEnvVar('SMTP_PASS') || 'TqUDx4xHBQffa835', 
  },
  tls: {
...
export const sendMail = async (jwtToken, email, user) => {
  try {
    ...
    const mailOptions = {
      from: `"Maryna Korniyenko" <${getEnvVar('SMTP_FROM') || 'marynakorniyenko@ukr.net'}>`,
      to: email,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p style="color: #666; font-size: 14px;">Best regards,<br>Maryna Korniyenko</p>
        </div>
      `,
    };
